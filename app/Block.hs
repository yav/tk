...
data Block = Block {
  ...
  pos   :: Vec2D Int,
  tr    :: T.Transform,
  locs  :: [(Vec2D Int,Action)]
} deriving Show

block :: [(Vec2D Int,Action)] -> Block
block as = Block { pos = 0, tr = mempty, locs = as, dim = foldl' upd 0 as }
  where
  upd m (p,_) = vop2 max m (p + 1)

positions :: Block -> [(Vec2D Int,Action)]
...
drawBlock b =
  P.pictures
    [ P.translate (cvt x) (cvt y) (drawAction sz a) | (Vec2D x y,a) <- positions b ]
  where
  sz = 32
...
parseBlock = fmap (block . concat . concat) . mapM parseLine . zip [0..] . lines
  where
  actLen = 4 -- 1 for space
  parseLine (row, cs) = zipWithM getA [0 ..] (chunk actLen cs) 
    where
    getA col xs =
      case take (actLen-1) xs of
        "..." -> pure []
        ys ->
          case parseAction ys of
            Just a  -> pure [(Vec2D col row, a)]
            Nothing ->
              Left ("Malformed action " ++ show (row+1) ++ ":" ++ show (1 + actLen * col) ++ ": " ++ ys)
